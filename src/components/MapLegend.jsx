const MapLegend = ({ mapType }) => {
    const map = mapTypesData[mapType]
    const maxValue = map.stops[map.stops.length - 1].value

    const gradientStops = map.stops
        .map((stop) => `${stop.color} ${(stop.value / maxValue) * 100}%`)
        .join(", ")

    return (
        <div className="bg-card/70 w-70 z-1000 p-3 absolute top-0 right-0 mr-4 mt-4 rounded-xl font-sans flex flex-col gap-2">
            <span className="text-sm font-medium">{map.title} ({map.unit})</span>
            <div
                className="w-full h-4 rounded-3xl"
                style={{ background: `linear-gradient(to right, ${gradientStops})` }}
            ></div>
            <div className="flex justify-between">
                <span>{map.stops[0].value} {map.unit}</span>
                <span>{maxValue} {map.unit}</span>
            </div>
        </div>
    )
}
